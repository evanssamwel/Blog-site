<?php
	...
	*/
    ...
    // Remove deprecated get_magic_quotes_gpc check and use stripslashes if needed (optional)
    if ($is_gpc && function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc()) {
        $string = stripslashes($string);
    }

    // Ensure DB connection
    if (!db_link()) {
        // Attempt to connect; you may want to call your db_connect() here directly
        db_connect();
        if (!db_link()) {
            // Connection failed — handle error or return original string to avoid further errors
            return $string;
        }
    }
    ...
    foreach ($specialChars as $char) {
        $countChar = substr_count($string, $char);
        $countEscChar = substr_count($string, '\\' . $char);
        if ($countChar != $countEscChar) $escaped = false;
        if ($countChar) $noSpecialChars = false;
    }

    if ($noSpecialChars) {
        // Check for unescaped backslashes
        $countBackslash = substr_count($string, '\\');
        $countDoubleBackslash = substr_count($string, '\\\\');
        if ($countBackslash != $countDoubleBackslash * 2) $escaped = false;
    }
